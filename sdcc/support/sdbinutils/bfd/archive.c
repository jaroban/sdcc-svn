...
_bfd_write_archive_contents(bfd* arch)
{
		...
		current = current->archive_next)
	{
		...
		bfd_size_type total_read_bytes = 0;
		bfd_size_type total_amount = 0;
		...
		while (remaining > 0)
		{
			...
			if (amt > remaining)
				amt = remaining;
			errno = 0;
			bfd_size_type read_bytes = bfd_bread(buffer, amt, current);
#if defined(_MSC_VER)
			// on windows, the read bytes will not match the file size because
			// CRLF gets replaced by '\n' when reading the file
			// here we subtract the deleted characters from the file size and check 
			// whether the read bytes match the corrected file size
		...
		if (total_read_bytes != total_amount)
		{
			if (bfd_get_error() != bfd_error_system_call)
