...
cl_51core::id_string(void)
{
	...
	for (i = 0;
		...
		i++);
	SPRINTF_S(id_string_51, sizeof(id_string_51), "%s %s",
	...
	if (cpu)
	{
		...
		else if (adpc > 0x7f)
		{
				...
				(mdpc != 0))
			{
				...
				dptr->decoders->add(banker);
				for (a = 0; a <= m; a++)
					banker->add_bank(a, dptr_chip, (t_addr)a * 2);
				banker->activate(0);
	...
	while (*b)
	{
		...
		if (*b == '%')
		{
			...
			case 'A': // absolute address
				...
				break;
			case 'l': // long address
				operand = (((t_addr)(rom->get(addr + 1))) << 8) | rom->get(addr + 2);
				temp.appendf(rom->addr_format, operand);
				addr_name(operand, rom, &temp);
				break;
...
cl_51core::print_regs(class cl_console_base* con)
{
			...
			if (dptr_chip)
			{
				for (i = 0; i <= mask; i++)
				{
					...
					for (di = dptr->get_size() - 1; di >= 0; di--)
						dp = (dp * 256) + dptr_chip->get((t_addr)a + di);
	...
	else
	{
		...
		data = xram->get((((t_addr)(sfr->read(DPH))) << 8) | sfr->read(DPL));
...
cl_51core::analyze(t_addr addr)
{
		...
		code != 0xa5 /* break point */)
	{
		...
		case 'A': // ajmp
			a = (addr & 0xf800) |
				(((t_addr)(rom->get(addr)) << 3) & 0x700) | 
				rom->get(addr + 1);
			addr = a;
			break;
		case 'l': // lcall
			a = ((t_addr)(rom->get(addr + 1)) << 8) | rom->get(addr + 2);
			analyze(a);
			addr = addr + tabl->length;
			break;
		case 'L': // ljmp
...
cl_uc51_cpu::init(void)
{
	...
	for (i = 0; i < 8; i++)
		acc_bits[i] = register_cell(bas, (t_addr)ACC + i);
