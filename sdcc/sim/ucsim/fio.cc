...
cl_f::init(void)
{
	...
	else if (!file_name.empty())
	{
		if (file_mode.empty())
			file_mode = "r+";
		if ((file_id = _open(file_name, open_flags(file_mode), (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH))) >= 0)
		{
			tty = _isatty(file_id);
			own = true;
			save_attributes();
...
cl_f::pick(void)
{
	...
	if (fp < 5)
		return 0;
	int i = _read(file_id, b, (fp > 101) ? 99 : fp - 1);
	if (i > 0)
	{
		int j;
		for (j = 0; j < i; j++)
...
cl_f::write(const char* buf, int count)
{
	...
	if (file_id >= 0)
	{
		if (type != F_SOCKET)
		{
			return _write(file_id, buf, count);
		}
